...
epoch = 50
batch = 32
denser = 100 ## in case model contains dense layer
regu = 0.1 ## in case of regularization
drop = 0.5
...
sparse_x = t.texts_to_matrix(doc_training, mode='count')
sparse_x_val = t.texts_to_matrix(doc_val, mode='count')
sparse_x_test = t.texts_to_matrix(doc_test, mode='count')
...
history = model.fit(sparse_x, y_train,                ## << HERE
                    validation_data=(sparse_x_val, y_val), ## << HERE
                    epochs=epoch, batch_size=batch,
                    callbacks=[F1History(train=(sparse_x,y_train),
                                         validation=(sparse_x_val,y_val)),checkpoint]
                    )
...
f1_sc = list(f1_sc)
f1_sc_val = list(f1_sc_val)
loss = list(loss)
val_loss = list(val_loss)

'''
NOTE: Model outputs a mean loss over all batches
        this loss is influenced by regularization and dropout
'''
diction = {'epoch':epochen,'f_train':f1_sc,'f_val':f1_sc_val,
...
for i in model_loads:  
    ...
    ## for training data
    loss_t =  model.evaluate(sparse_x, y_train, batch_size=batch, verbose=0)  # gives model metrics
    loss_tr.append(loss_t[0])

    y_pred_train =  model.predict(sparse_x, verbose=0)  # gives probabilities
    ...
    ## for validation data
    y_prob =  model.evaluate(sparse_x_val, y_val, verbose=0)  # gives model metrics
    loss_ev.append(y_prob[0])
    y_pred_val = model.predict(sparse_x_val)
...
print("Fscore",Fscore_train)
print("Loss",loss)
print("Fscore Val",Fscore_val)
...
saved_train_preds = pd.DataFrame(saved_train_preds, columns=['y_train_preds'])
saved_train_preds.to_csv(os.path.join(savemodelpath,'saved_train_preds.txt'),sep=',',encoding='utf-8')


''' testing best model (highest validation score)'''
...
y_pred_test = model.predict(sparse_x_test)
...
test_scores = {'f_test':f_test,'pr_test':pr_test,'re_test':re_test,
               'tp_test':tp_test, 'fp_test':fp_test, 'fn_test':fn_test, 'best_model':loadpath_best}
...
def predict_prob(string):
    ...
    '''
    global model
    x_temp = t.texts_to_matrix(string, mode='count')
...
#explanation = explainer.explain_instance(x_val[1], pipeline.predict_proba, num_features=10)
explainer = LimeTextExplainer(class_names=class_names)

def weight_mtx(data, sparse_data):
    ...
    for i in sample_indices:
        print(i)
        explanations.append(explainer.explain_instance(
                data[i], predict_prob, num_features=10))
        temp = sparse_data[i]
...
## once for positive and once for negative class
## and once for both
...
temp = {'doc':doc_test,'label':y_test_list}
...
one_doc_sparse = t.texts_to_matrix(one_doc, mode='count')
...
zero_doc_sparse = t.texts_to_matrix(zero_doc, mode='count')
...
both_doc_sparse = t.texts_to_matrix(both_doc, mode='count')

unique, counts = np.unique(both,                 
                           return_counts=True)
print(unique)
print(counts)

form = list(test_all['Form'])
form = form[0:percentage]
unique, counts = np.unique(form,                 
                           return_counts=True)
...
''' negative class '''
df_w, prob_one = weight_mtx(zero_doc,zero_doc_sparse)
...
''' positive class '''
df_w, prob_one = weight_mtx(one_doc,one_doc_sparse)
...
''' both classes '''
df_w, prob_one = weight_mtx(both_doc,both_doc_sparse)
...
temp = {'doc':doc_training,'label':y_train_list}
df_explainer = pd.DataFrame(temp)
...
''' 
Abs importance values are for positive and negative classes
since we do not want importance alues to cancel each other out.
'''
