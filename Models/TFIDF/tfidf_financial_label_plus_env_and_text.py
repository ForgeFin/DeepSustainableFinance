...
def create_model(loadweight=None):
    ...
    den = Dense(denser,# kernel_regularizer=l1(0.0001),
    ...
    combined = Concatenate()([den, env_perf])
...
history = model.fit([sparse_x,golden_env_train], y_train,                ## << HERE
                    validation_data=([sparse_x_val,golden_env_val], y_val), ## << HERE
...
for i in model_loads:
    ...
    # for training data
    y_pred_train =  model.predict([sparse_x,golden_env_train], verbose=0)  # gives probabilities
    ...
    ## for validation data
    y_pred_val = model.predict([sparse_x_val,golden_env_val])
...
y_pred_test =  model.predict([sparse_x_test,golden_env_test],
                              verbose=0)  # gives probabilities
    ...
    for i in sample_indices:
        ...
        def predict_prob(string):
            ...
            for n in range(0,shape_n):
                ...
                golden.append(golden_env_test[i])
            golden = np.array(golden, dtype='float32')
            
            prediction = model.predict([x_temp,golden])
        ...
        gld = np.array(golden_env_test[i], dtype='float32')
        gld = gld.reshape((1,1))
        gld = gld.astype('float32')
                
        prediction = model.predict([temp,gld])
...
gld_test_list = list(golden_env_test)
temp = {'doc':doc_test,'label':y_test_list,'gld':gld_test_list}
...
gld_one = df_one_class['gld'].iloc[:percentage]
gld_one = np.array(gld_one, dtype='float32')
...
gld_zero = df_zero_class['gld'].iloc[:percentage]
gld_zero = np.array(gld_zero, dtype='float32')
...
''' both classes '''
df_w, prob_one = weigth_mtx_test(one_doc,one_doc_sparse)
...
def weigth_mtx_neg_test(data, sparse_data):
    ...
    for i in sample_indices:
        ...
        gld = np.array(gld_zero[i], dtype='float32')
        gld = gld.reshape((1,1))
        gld = gld.astype('float32')
...
def weigth_mtx_pos_test(data, sparse_data):
    ...
    for i in sample_indices:
        ...
        gld = np.array(gld_one[i], dtype='float32')
        gld = gld.reshape((1,1))
        gld = gld.astype('float32')
    ...
    for i in sample_indices:
        ...
        def predict_prob(string):
            ...
            for n in range(0,shape_n):
                ## for length of string (shape_n), create market
                ## filled with instance i
                golden.append(golden_env_train[i])
            golden = np.array(golden, dtype='float32')
        ...
        temp = temp.reshape((1,len(temp)))
        temp = temp.astype('float32')
        gld = np.array(golden_env_train[i], dtype='float32')
        gld = gld.reshape((1,1))
        gld = gld.astype('float32')
...
gld_train_list = list(golden_env_train)
temp = {'doc':doc_training,'label':y_train_list,'gld':gld_train_list}
df_explainer = pd.DataFrame(temp)
